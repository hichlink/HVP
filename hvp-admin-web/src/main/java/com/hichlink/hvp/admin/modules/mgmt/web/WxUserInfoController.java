...
import com.hichlink.hvp.common.service.WxUserInfoService;

/**
 * 
 * <b>Title：</b>WxUserInfoController.java<br/>
 * <b>Description：</b> <br/>
...
public class WxUserInfoController extends BaseController {
	private static final Logger LOG = LoggerFactory.getLogger(WxUserInfoController.class);
	@Autowired
	...
	@ResponseBody
	public Map<String, Object> pageQuery(Page<WxUserInfo> page) {
		Page<WxUserInfo> list = wxUserInfoService.pageQuery(page);
		return super.page(list);
	}

	@RequestMapping(value = "/add.ajax", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> add(@ModelAttribute("wxUserInfo") WxUserInfo data) {
		wxUserInfoService.saveAndUpdate(data);
	...
	public Map<String, Object> delete(Long userId) {
		wxUserInfoService.delete(userId);
	...
	public Map<String, Object> get(Long userId) {
		WxUserInfo data = wxUserInfoService.get(userId);
	 ...
	 * 要谨慎使用
	 */
	@ModelAttribute("wxUserInfo")
	public void getForUpdate(@RequestParam(value = "updId", required = false) Long updId, Model model) {
		if (null != updId) {
			model.addAttribute("wxUserInfo", wxUserInfoService.get(updId));
		} else {
			model.addAttribute("wxUserInfo", new WxUserInfo());
		}
	}
}